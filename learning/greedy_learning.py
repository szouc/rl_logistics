...
def greedy_costs_learning(env, num_episodes):
...
def greedy_times_learning(env, num_episodes):
    ...
    for i_episode in range(num_episodes):
        ...
        for t in itertools.count():
            ...
            (order_flags, _, vehicle_flags, vehicle_costs) = state
            ...
            # rest_o_index = np.random.choice(np.nonzero(order_flags)[0])
            rest_o_index = np.random.choice(len(order_flags))
            vehicle_flags_array = np.array(vehicle_flags)
            for _ in range(len(vehicle_costs)):
                min_v_index = np.argmin(vehicle_flags_array)
                if vehicle_costs[min_v_index] >= 5:
                    vehicle_flags_array[min_v_index] += 99
                    continue
                else:
                    action = (rest_o_index, min_v_index)
                    break
